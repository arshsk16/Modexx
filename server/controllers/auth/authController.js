...
const registerUser = async (req, res) => {
  try {
    ...
    if (type === "user") {
      ...
      try {
        ...
        if (!results || !results.length) {
          // If geocoding fails, use default coordinates (center of India) or allow registration without coordinates
          console.warn(`Geocoding failed for postal code: ${address.postalCode}, using default coordinates`);
        ...
        // If geocoding service fails, use default coordinates and continue with registration
    ...
    res.status(500).json({ 
      message: "Error registering user/hospital", 
