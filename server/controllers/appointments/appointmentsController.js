...
const addEmergencyAppointment = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!results || results.length === 0) {
      ...
      console.warn(`Geocoding failed for postal code: ${pincode}, using default coordinates`);
      userLat = 20.5937; // Default to center of India
      userLong = 78.9629;
    } else {
      userLat = results[0].latitude;
      userLong = results[0].longitude;
    }
  } catch (geocodeError) {
    // If geocoding service fails, use default coordinates and continue
  ...
  if (!hospitals || hospitals.length === 0) {
    return res.status(404).json({ message: "No hospitals found in the system" });
  }
  ...
  for (let hospital of hospitals) {
    ...
    try {
      ...
      console.error(`Error calculating route for hospital ${hospital._id}:`, routeError);
      // Continue to next hospital if route calculation fails
    }
  }

  // Fallback: if routing failed for all hospitals, pick the first hospital to avoid 500
  if (!nearestHospital) {
    console.warn("No route found to any hospital, falling back to first hospital");
