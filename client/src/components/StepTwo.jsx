...
function StepTwo() {
  ...
  const handleCommaSeparatedValues = (e) => {
    ...
    setOtherDetails((prevDetails) => ({
      ...prevDetails,
      [name]: value.trim().split(',').map(item => item.trim()).filter(item => item.length > 0),
    }));
    setErrors((prev) => ({
      ...prev,
      frontend: {
        ...
        <>
          ...
          <div className="form-section">
            ...
            <select
              id="department"
              name="department"
              value={Array.isArray(otherDetails.department) ? otherDetails.department[0] || '' : (otherDetails.department || '')}
              onChange={(e) => {
                setOtherDetails({
                  ...otherDetails,
