...
if (window.location.hostname === 'localhost') {
  // If localhost, use the local backend (server runs on port 5000)
...
// Helper function to dynamically replace 'localhost:5000' in any API request with the production URL when deployed
const adjustUrl = (url) => {
  if (window.location.hostname !== 'localhost' && url.includes('localhost:5000')) {
    ...
    return url.replace('localhost:5000', 'https://medi-connect-f671.onrender.com');
  }
