...
const Chatbot = ({isOpen, setIsOpen}) => {
  ...
  const about_message =
    'At Modex Health, we envision a world where accessing outpatient care is as simple as a few clicks. By leveraging technology and innovation, we aim to provide a platform that bridges the gap between patients and healthcare providers, making high-quality care accessible to everyone, anywhere.';
  const loadConfig = useCallback(async () => {
    try {
      // Load from env; if not set, keep empty and block calls
  ...
  const generateResponse = async (message) => {
    if (!apiKey || !apiUrl) {
      return 'API key is not configured. Please set REACT_APP_GEMINI_API_KEY.';
    }
