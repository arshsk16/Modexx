...
const HospitalsList = () => {
  ...
  const handleBooking = async (hospitalId) => {
    try {
      ...
      // If the hospital id is not a valid MongoDB ObjectId, skip booking (local/static hospitals)
      if (!hospitalId || hospitalId.length !== 24) {
        alert('This hospital is sample data. Please choose a listed hospital to book.');
        return;
      }

      // Basic client-side validation to avoid 400 from backend
      if (!bookingData.date) {
